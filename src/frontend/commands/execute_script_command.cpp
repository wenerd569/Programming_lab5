...
ExecuteScriptCommand::ExecuteScriptCommand(std::shared_ptr<IOInterface> ioInterface, std::shared_ptr<CommandManager> comandManager, std::shared_ptr<CollectionManager> collectionManager) :
    Command(ioInterface, comandManager, collectionManager) {};

void ExecuteScriptCommand::execute(std::vector<std::string>& args){
    std::string filePath;
    if(!getOneStringArg(args, filePath)) return;
    try{
