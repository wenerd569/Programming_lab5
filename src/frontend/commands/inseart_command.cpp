...
InseartCommand::InseartCommand(std::shared_ptr<IOInterface> ioInterface, std::shared_ptr<CommandManager> comandManager, std::shared_ptr<CollectionManager> collectionManager):
Command(ioInterface, comandManager, collectionManager) {};

void InseartCommand::execute(std::vector<std::string>& args){
    size_t index;
    if (!getOneNumArg<size_t>(args, index)) return;
